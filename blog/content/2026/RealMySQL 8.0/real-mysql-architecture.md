...
- 쿼리 파서
	- 쿼리를 토큰으로 분리해 트리 형태의 구조로 만듦
- 전처리기
	- 파서 트리를 기반으로 구조적인 문제점이 있는지 확인
- 옵티마이저
...
- Change Buffer
	- 변경해야 할 인덱스 페이지가 버퍼 풀에 있으면 바로 업데이트를 수행하지만 그렇지 않고 디스크로부터 읽어와서 업데이트 해야 한다면 임시 공간에 저장해 두고 바로 사용자에게 결과를 반환하는 형태로 성능을 향상시키게 되는데 이때 임시 메모리 공간을 Change Buffer 라고 한다.
		- Change Buffer는 추후 Background Thread에 의해 merge(병합)된다.
